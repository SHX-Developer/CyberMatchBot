...
from app.keyboards import language_keyboard, profile_panel_keyboard, settings_keyboard
...
async def profile_handler(
    ...
    username = (
        f'@{user.username}'
        if user.username
        else i18n.t(locale, 'profile.username.missing')
    )

    language_code = user.language_code.value if user.language_code else i18n.default_locale
    language_name = i18n.t(locale, f'profile.language.{language_code}')
    ...
    await message.answer(
        ...
        reply_markup=profile_panel_keyboard(i18n, locale),
    )
...
async def profile_stats_handler(callback: CallbackQuery, session: AsyncSession, i18n: LocalizationManager) -> None:
    if callback.from_user is None:
        return
